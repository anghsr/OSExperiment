...
public class HRRN {  
    ...
    public static  void HRRN(){  
        ...
        {  
            ...
            System.out.print("Task:"+tm.format(new Date())+"第"+(int)tem[0]+"号进程开始运行");  
            try {  
                Thread.sleep((long) tem[3]*1000);//模拟进程执行所需要的时间  
            } catch (InterruptedException e) {  
                e.printStackTrace();  
            }  
            System.out.println("\n====="+tm.format(new Date())+"进程结束运行\n=====服务时间："+(int)tem[3]+"S");  
            zz[i] = (int)tem[3];
        ...
        System.out.println("-------------------------------------------------");
        show_time(zz);//显示每个进程的周转时间  
        System.out.println("-------------------------------------------------");
    }  
  
    public  static void show_time(int[] zz)//显示每个进程的周转时间  
    {   
        ...
        {  
            double[] t=execute_time.get(i);  
            System.out.println("task:"+t[0]+":带权周转时间="+(int)(t[1]/(1000*zz[i]))+"S");  
            sum_time+=(t[1]/zz[i]);  
        }  
